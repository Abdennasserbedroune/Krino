    ...
    <div style={{ minHeight: '100dvh', background: '#F7F3EF', fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflowX: 'hidden' }}>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        .pw-input { height: 44px; width: 100%; padding: 0 14px; border-radius: 12px; border: 1.5px solid rgba(17,24,39,0.14); background: rgba(17,24,39,0.02); font-size: 14px; font-family: 'Inter', sans-serif; color: #111827; outline: none; transition: border-color 150ms ease, background 150ms ease; }
        ...
        .pw-submit-btn { margin-top: 6px; height: 48px; width: 100%; border-radius: 9999px; color: #fff; font-size: 15px; font-weight: 500; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: opacity 150ms ease, transform 150ms ease, background 300ms ease; font-family: 'Inter', sans-serif; }
        .pw-submit-btn:not(:disabled):hover { transform: translateY(-1px); }
        .pw-submit-btn:disabled { opacity: 0.55; cursor: not-allowed; }
        .pw-google-btn { width: 100%; height: 44px; border-radius: 9999px; border: 1.5px solid rgba(17,24,39,0.14); background: #fff; display: flex; align-items: center; justify-content: center; gap: 10px; font-size: 14px; font-weight: 500; color: #374151; cursor: pointer; transition: border-color 150ms, transform 150ms; font-family: 'Inter', sans-serif; }
        ...
        <div style={{ borderRadius: 31, background: '#FFFFFF', boxShadow: '0 0 0 1px rgba(0,0,0,0.06), 0 24px 24px -12px rgba(0,0,0,0.06)', padding: '36px 36px 32px', overflow: 'hidden', position: 'relative' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              ...
              <input id="reg-name" type="text" required placeholder="Your full name" value={name} onChange={e => setName(e.target.value)} className={`pw-input pw-input-${role}`} autoComplete="name"/>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <label htmlFor="reg-email" style={{ fontSize: 13, fontWeight: 500, color: '#374151' }}>Email</label>
              <input id="reg-email" type="email" required placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} className={`pw-input pw-input-${role}`} autoComplete="email"/>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              <label htmlFor="reg-password" style={{ fontSize: 13, fontWeight: 500, color: '#374151' }}>Password</label>
              <input id="reg-password" type="password" required placeholder="e.g. MyPass1@" value={password} onChange={e => setPassword(e.target.value)} className={`pw-input pw-input-${role}`} autoComplete="new-password"/>
