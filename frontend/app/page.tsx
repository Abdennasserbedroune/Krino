...
function useLoopingTypewriter(phrases: string[], typeSpeed = 72, deleteSpeed = 36, pauseMs = 1800) {
  ...
  useEffect(() => {
    ...
    function typePhrase(phrase: string, pos: number) {
      ...
      if (pos <= phrase.length) {
        setDisplayed(phrase.slice(0, pos));
        setDone(pos === phrase.length);
        timeout = setTimeout(() => typePhrase(phrase, pos + 1), typeSpeed);
      } else {
        // pause then erase
        timeout = setTimeout(() => erasePhrase(phrase, phrase.length), pauseMs);
      }
    }
    ...
    function erasePhrase(phrase: string, pos: number) {
      ...
      if (pos >= 0) {
        setDisplayed(phrase.slice(0, pos));
        setDone(false);
        timeout = setTimeout(() => erasePhrase(phrase, pos - 1), deleteSpeed);
      } else {
        indexRef.current = (indexRef.current + 1) % phrasesRef.current.length;
        timeout = setTimeout(() => typePhrase(phrasesRef.current[indexRef.current], 0), 200);
      }
    }

    typePhrase(phrasesRef.current[indexRef.current], displayed.length || 0);
...
function SeekerScoreVisual({ accent }: { accent: string }) {
  const r = 38, circ = 2 * Math.PI * r, score = 73, dash = (score / 100) * circ;
  return (
        ...
        {[['Keywords', 91],['Format', 88],['Length', 74],['Sections', 56]].map(([label, val]) => (
          <div key={label as string} className="flex items-center gap-2">
            ...
            <div style={{ width: 64, height: 4, borderRadius: 9999, background: 'rgba(17,24,39,0.08)' }}>
              <div style={{ width: `${val}%`, height: '100%', borderRadius: 9999, background: accent }}/>
            </div>
...
function SeekerMatchVisual({ accent }: { accent: string }) {
  const r = 42, circ = 2 * Math.PI * r, score = 84, dash = (score / 100) * circ;
  return (
...
function HRBlindVisual({ accent }: { accent: string }) {
  const r = 40, circ = 2 * Math.PI * r, score = 91, dash = (score / 100) * circ;
  return (
    <div className="flex items-center justify-center h-full gap-5">
      <div className="relative w-24 h-24 flex-shrink-0">
        <svg viewBox="0 0 100 100" className="w-24 h-24 -rotate-90">
...
// ─── Card component ───────────────────────────────────────────────────────────
function FeatureCard({ num, title, desc, visual, accent }: {
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          <p style={{ margin: '0 0 4px', fontSize: 13, fontWeight: 500, color: highlighted ? '#9CA3AF' : '#6B7280' }}>{name}</p>
...
export default function LandingPage() {
  ...
  const phrases = mode === 'seeker' ? SEEKER_PHRASES : RECRUITER_PHRASES;
  const { displayed: typedText, done: typeDone } = useLoopingTypewriter(phrases, 72, 36, 1800);
  ...
  const updateIndicator = (m: Mode) => {
    const btn  = m === 'seeker' ? seekerRef.current : recruiterRef.current;
    const pill = pillRef.current;
    if (!btn || !pill) return;
    const br = btn.getBoundingClientRect();
    const pr = pill.getBoundingClientRect();
    setIndicatorStyle({ left: br.left - pr.left, width: br.width });
  };

  useEffect(() => { updateIndicator(mode); }, [mode]);
  useEffect(() => { updateIndicator('seeker'); }, []);
  ...
  const SEEKER_PLANS = [
    {
      ...
      cta: 'Get started free', ctaHref: '/auth/register', highlighted: false,
    },
    {
      ...
      cta: 'Start Pro free trial', ctaHref: '/auth/register?plan=pro', highlighted: true,
    },
    {
      ...
      cta: 'Talk to us', ctaHref: '/auth/register?plan=teams', highlighted: false,
    },
  ];
  ...
  const RECRUITER_PLANS = [
    {
      ...
      cta: 'Start screening free', ctaHref: '/auth/register', highlighted: false,
    },
    {
      ...
      cta: 'Start Growth trial', ctaHref: '/auth/register?plan=growth', highlighted: true,
    },
    {
      ...
      cta: 'Contact sales', ctaHref: '/auth/register?plan=enterprise', highlighted: false,
    },
  ];
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#F7F3EF', fontFamily: "'Inter', sans-serif", overflowX: 'hidden' }}>
      ...
      <header style={{
        ...
        <div className="flex items-center gap-2">
          <Link href="/auth/login" style={{ fontSize: 13, fontWeight: 500, color: '#6B7280', padding: '8px 16px', borderRadius: 9999, textDecoration: 'none', transition: 'color 150ms ease' }}
      ...
      <main style={{ position: 'relative', zIndex: 1 }}>
        ...
        <section style={{
          ...
          <div style={{
            ...
            border: '1px solid rgba(17,24,39,0.09)', boxShadow: '0 1px 4px rgba(17,24,39,0.06)',
            marginBottom: 36,
          }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 500, color: '#6B7280' }}>Trusted by 2,800+ job seekers & recruiters</span>
          </div>

          {/* mode toggle */}
          <div
            ref={pillRef}
            role="group" aria-label="Select user mode"
            style={{
              position: 'relative', display: 'inline-flex', alignItems: 'center',
              background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(8px)',
              border: '1px solid rgba(17,24,39,0.10)', borderRadius: 9999,
              padding: 4, marginBottom: 40, boxShadow: '0 1px 4px rgba(17,24,39,0.06)',
            }}
          >
            <div aria-hidden style={{
              position: 'absolute', top: 4, height: 'calc(100% - 8px)',
              borderRadius: 9999, background: accent.color,
              ...
              zIndex: 0, boxShadow: `0 2px 8px ${accent.color}44`,
            }}/>
            <button ref={seekerRef} onClick={() => setMode('seeker')} aria-pressed={mode === 'seeker'}
              style={{ position: 'relative', zIndex: 1, padding: '7px 18px', borderRadius: 9999, fontSize: 13, fontWeight: 500, color: mode === 'seeker' ? '#fff' : '#6B7280', background: 'transparent', border: 'none', cursor: 'pointer', transition: 'color 200ms ease', whiteSpace: 'nowrap' }}
            >Job Hunter</button>
            <button ref={recruiterRef} onClick={() => setMode('recruiter')} aria-pressed={mode === 'recruiter'}
              style={{ position: 'relative', zIndex: 1, padding: '7px 18px', borderRadius: 9999, fontSize: 13, fontWeight: 500, color: mode === 'recruiter' ? '#fff' : '#6B7280', background: 'transparent', border: 'none', cursor: 'pointer', transition: 'color 200ms ease', whiteSpace: 'nowrap' }}
            >HR Teams</button>
          </div>
          ...
          <h1 style={{
            ...
            letterSpacing: '-0.025em', color: '#111827', maxWidth: 900,
          }}>
            {mode === 'seeker' ? (
              <>Is your resume<br/>
                <span style={{ color: accent.color }}>
                  {typedText}{!typeDone && <span className="tw-cursor" aria-hidden>|</span>}
                </span>
              </>
            ) : (
          ...
          <p style={{
            ...
            lineHeight: 1.7, color: '#6B7280', maxWidth: 580, letterSpacing: '0.01em',
          }}>
            {mode === 'seeker'
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center">
            <Link href="/auth/register" style={{
              ...
              fontSize: 15, fontWeight: 500, letterSpacing: '0.01em', textDecoration: 'none',
              boxShadow: 'rgba(0,0,0,0.4) 0px 12px 24px -6px, rgba(255,255,255,0.15) 0px 1px 1px 0px inset, rgba(0,0,0,0.5) 0px -2px 3px 0px inset, rgba(0,0,0,0.10) 0px 0px 0px 1px',
              transition: 'transform 150ms ease, opacity 150ms ease',
            }}
            ...
            <Link href="#features" style={{
              ...
              background: 'transparent', border: '1px solid rgba(17,24,39,0.14)',
              color: '#4B5563', fontSize: 14, fontWeight: 500, textDecoration: 'none',
          ...
          <div style={{ marginTop: 48, display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap', justifyContent: 'center' }}>
            {[{ val: '75%', label: 'ATS rejection rate' },{ val: '2.8K', label: 'Resumes analysed' },{ val: '94%', label: 'Pass-rate improvement' }].map(s => (
        ...
        <section id="features" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 100px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#9CA3AF' }}>
              {mode === 'seeker' ? 'Job Hunter Features' : 'HR Teams Features'}
            </span>
            <div style={{ height: 1, flex: 1, background: 'rgba(17,24,39,0.08)' }}/>
          </div>

          {mode === 'seeker' ? (
            // Seeker: 3-col top row + full-width bottom
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }} className="bento-seeker">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0 }} className="bento-top">
                {SEEKER_CARDS.slice(0, 3).map(card => (
                  <div key={card.num} style={{ padding: 6, height: 340 }}>
                    <FeatureCard {...card} accent={accent.color}/>
                  </div>
                ))}
              </div>
              <div style={{ padding: 6 }}>
                <FeatureCard {...SEEKER_CARDS[3]} accent={accent.color}/>
              </div>
            </div>
          ) : (
            // Recruiter: 2+1 top row + 1+1 bottom row, zero gap
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gridTemplateRows: 'auto auto', gap: 0 }} className="bento-recruiter">
              <div style={{ gridColumn: 'span 2', padding: 6, minHeight: 320 }}>
                <FeatureCard {...RECRUITER_CARDS[0]} accent={accent.color}/>
              </div>
              <div style={{ gridColumn: 'span 1', padding: 6, minHeight: 320 }}>
                <FeatureCard {...RECRUITER_CARDS[1]} accent={accent.color}/>
              </div>
              <div style={{ gridColumn: 'span 1', padding: 6, minHeight: 300 }}>
                <FeatureCard {...RECRUITER_CARDS[2]} accent={accent.color}/>
              </div>
              <div style={{ gridColumn: 'span 1', padding: 6, minHeight: 300 }}>
                <FeatureCard {...RECRUITER_CARDS[3]} accent={accent.color}/>
              </div>
              {/* empty cell to fill 3rd column on bottom row */}
              <div style={{ gridColumn: 'span 1', padding: 6, minHeight: 300 }}>
                <div style={{
                  height: '100%', borderRadius: 32,
                  background: 'linear-gradient(160deg, rgba(249,115,22,0.04) 0%, rgba(249,115,22,0.01) 100%)',
                  border: '1.5px dashed rgba(249,115,22,0.18)',
                  display: 'flex', flexDirection: 'column',
                  alignItems: 'center', justifyContent: 'center', gap: 8,
                }}>
                  <span style={{ fontSize: 22 }}>🚀</span>
                  <span style={{ fontSize: 13, color: '#9CA3AF', fontWeight: 500 }}>More coming soon</span>
                  <span style={{ fontSize: 11, color: '#D1D5DB' }}>ATS integrations & more</span>
                </div>
              </div>
            </div>
          )}
        ...
        <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 100px' }}>
          <div style={{ padding: 1, borderRadius: 32, background: 'linear-gradient(135deg, rgba(255,255,255,0.92) 0%, rgba(17,24,39,0.07) 100%)' }}>
            <div style={{
              borderRadius: 31, background: '#FFFFFF',
              boxShadow: '0 0 0 1px rgba(0,0,0,0.06), 0 12px 32px rgba(0,0,0,0.06)',
              padding: '48px',
              display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0,
            }} className="social-grid">
              {[
                { quote: '"Krino showed me in seconds why I kept getting ghosted. 3 keywords missing. Fixed them. Got 5 interviews in a week."', name: 'Amira K.', role: 'Software Engineer', avatar: '#3b82f6', initials: 'AK' },
                { quote: '"We cut screening time from 6 hours to 40 minutes per role. The blind review mode is now a requirement for every hire."', name: 'Jason T.', role: 'Head of Talent, Series B startup', avatar: '#f97316', initials: 'JT' },
                { quote: '"My ATS score went from 51 to 89 in one session. The keyword gap view is the most useful job-search tool I have ever seen."', name: 'Priya M.', role: 'Product Manager', avatar: '#22c55e', initials: 'PM' },
              ].map((t, i, arr) => (
                <div key={t.name} style={{ padding: '0 36px', borderRight: i < arr.length - 1 ? '1px solid rgba(17,24,39,0.07)' : 'none' }} className="testimonial-item">
                  <p style={{ margin: '0 0 20px', fontSize: 14, color: '#4B5563', lineHeight: 1.75, fontStyle: 'italic' }}>{t.quote}</p>
                  <div className="flex items-center gap-3">
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: t.avatar, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#fff' }}>{t.initials}</div>
                    <div>
        ...
        {/* ── PRICING ── */}
        <section id="pricing" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 100px' }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            ...
            <h2 style={{ margin: '0 0 12px', fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 500, color: '#111827', letterSpacing: '-0.025em', lineHeight: 1.1 }}>Simple, transparent pricing.</h2>
            <p style={{ fontSize: 16, fontWeight: 300, color: '#6B7280', letterSpacing: '0.01em', lineHeight: 1.7 }}>
              {mode === 'seeker' ? 'Start free. Upgrade when you need more analyses or AI rewrites.' : 'Start free with one role. Scale when your pipeline grows.'}
            </p>
          </div>
          <div style={{ display: 'flex', gap: 12, alignItems: 'stretch' }} className="pricing-grid">
            {plans.map(plan => <PricingCard key={plan.name} {...plan} accent={accent.color}/>)}
          </div>
          <p style={{ textAlign: 'center', marginTop: 24, fontSize: 12, color: '#9CA3AF' }}>All plans include a 14-day free trial. No credit card required to start.</p>
        </section>

        {/* ── CTA ── */}
        <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 120px' }}>
          <div style={{ padding: 1, borderRadius: 32, background: 'linear-gradient(135deg, rgba(255,255,255,0.90) 0%, rgba(17,24,39,0.08) 100%)' }}>
            <div style={{
              borderRadius: 31, background: '#111827',
              ...
              <Link href="/auth/register" style={{
                ...
                transition: 'transform 150ms ease', position: 'relative', zIndex: 1,
              }}
                onMouseEnter={e => (e.target as HTMLElement).style.transform = 'translateY(-2px)'}
                onMouseLeave={e => (e.target as HTMLElement).style.transform = 'translateY(0)'}
              >
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .bento-recruiter { grid-template-columns: 1fr !important; }
          .bento-recruiter > div[style*="span 2"] { grid-column: span 1 !important; }
        ...
        @media (max-width: 1024px) and (min-width: 769px) {
          .bento-top { grid-template-columns: repeat(2, 1fr) !important; }
          .bento-recruiter { grid-template-columns: repeat(2, 1fr) !important; }
          .bento-recruiter > div[style*="span 2"] { grid-column: span 2 !important; }
